function FlowData = FlowComparison_HN(Res)
    %% Function to compare flows for different scenario %%    
    
    % Assign a structure to hold all data
    FlowData = struct;
    counter = 1;
    ZoomSites = readtable(Res(1).ZoomBaseflow,'delimiter',',');
    ...
    for i = 1:size(Res(1).QIndex,1)
        ...
        for j = 1:size(Res,2)    
            FVQIdx = find(contains(Res(j).Flow.Properties.VariableNames,strcat('NS',num2str(Res(j).QIndex.NSnum(i,1)),'_FLOW_m_3S')));    
    ...
    for i = 1:size(FlowData,2)
        ...
        xlabel('Date');
        ylabel('Flow (ML/day)');
        StatBox = annotation('textbox',[0.1,0.8,0.25,0.17],'String',Stats); % Place the stats text box on top left corner of figure
        StatBox.FontSize = 15;
        ...
        if Res(1).IsZoomFlow == 1
            if ismember(FlowData(i).Site,ZoomSites.Name) == 1
                ZoomRow = find(strcmp(FlowData(i).Site,ZoomSites.Name));
                ZoomYLim = ZoomSites.YLim(ZoomRow,1);
                % Zoomed Timeseries figure block
                QZoomfig = figure('units','normalized','outerposition',[0 0 1 1]);
                plot(FlowData(i).Flow.Date,FlowData(i).Flow.(1),'LineStyle',':','color','black','LineWidth',1);
                title(['Flows - ',strrep(FlowData(i).Site,'_',' ')]);
                hold on
                for j = 2:size(Res,2)
                ...
                xlabel('Date');
                ylabel('Flow (ML/day)');                
                saveas(QZoomfig,['Zoomed Flows at ',FlowData(i).Site],'jpg');
            end
        end
        close all   
    end
