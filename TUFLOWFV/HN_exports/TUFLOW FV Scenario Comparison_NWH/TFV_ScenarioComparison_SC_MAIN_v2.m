...
ScriptDir = 'N:\Asset Knowledge\Manage Projects\2 Current\20037702\3_Data\Scripts\TUFLOW FV Scenario Comparison_NWH\';
cd(ScriptDir);
addpath(genpath(ScriptDir));

% Parent Directory where all necessary TUFLOW MATLAB scripts are, such as
% netcdf_get_var
SupportingScriptsDir = 'N:\Asset Knowledge\Manage Projects\2 Current\20037702\3_Data\Scripts\';
addpath(genpath(SupportingScriptsDir));

ScnUIindexFiles = {'Index_Scn05_Scn00_Scn00_DRY_EAS.csv'};   

for iScnIdx = 1:1
    sprintf('Running Scenario set %d of 10',iScnIdx)
    ...
    for i = 1:size(UIindex,1)
        ...
        Res(i).QIndex = QIndex;
        Res(i).Flow = readtable(UIindex.FluxFile{i,1},'delimiter',',',"VariableNamingRule","preserve");
