function Data2 = WQ_TimeseriesComparison_SC(Res)
    ...
    for i = 1:size(SiteNames,2)        
        for j = 1:size(Res(1).Params,1)        
            ...
            plot(Table.Date,Table.EEStrigger,'color','magenta','LineStyle','--','LineWidth',1.2);
            ax = gca;
            ax.FontSize = 22;
            MaxOfTable = max(max(Table{:,2:end}));
            NewYLim = MaxOfTable + (MaxOfTable*0.15);
            ylim([0 NewYLim]);
            if strcmp(ParamStdName,'WL') == 1
               ylim([-inf NewYLim]);
            end
            xlabel('Date');
            ...
            BoxPlot = boxplot(table2array(Table(:,2:end-2)),'BoxStyle','outline','Colors','kbgry',...
            ...
            BoxTrigger1 = plot(BoxTck,repmat(TriggerValue1,1,size(BoxTck,2)),'m--*','LineWidth',1.2);
            BoxGCA.FontSize = 22;
            ylim([0 NewYLim]);
            if strcmp(ParamStdName,'WL') == 1
               ylim([-inf NewYLim]);
            ...
            ylabel([ParamStdName ' ' ParamUnit]);
            hold off;           
            legend({'ANZG DGV','EES WQO'},'Location','northeastoutside');
            if strcmp(ParamStdName,'Enterococci') == 1
                legend({'NHMRC DGV','EES WQO'},'Location','northeastoutside');
            end
