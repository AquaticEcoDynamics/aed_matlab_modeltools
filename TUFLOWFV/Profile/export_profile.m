function [data,depth,outdate] = export_profile(ncfile,mdate,X,Y,var)
data = [];
depth = [];
outdate = [];
...
[outdate,dateint] = min(abs(double(dat.Time) - mdate));

mod = tfv_readnetcdf(ncfile,'timestep',dateint);

pnt(1,1) = X;
pnt(1,2) = Y;

geo_x = double(mod.cell_X);
geo_y = double(mod.cell_Y);
dtri = DelaunayTri(geo_x,geo_y);


pt_id = nearestNeighbor(dtri,pnt);

Cell_3D_IDs = find(mod.idx2==pt_id);

surfIndex = min(Cell_3D_IDs);
botIndex = max(Cell_3D_IDs);

depth(1)  = mod.layerface_Z(surfIndex + pt_id - 1);
for i = 1 : mod.NL(pt_id)
    % mid point of layer
    depth(i+1) = (mod.layerface_Z(Cell_3D_IDs(i) + pt_id-1) + mod.layerface_Z(Cell_3D_IDs(i) + pt_id-1 +1))/2.;
end
depth(length(Cell_3D_IDs)+2)  = mod.layerface_Z(botIndex+pt_id-1+1);

data(1,:) = mod.(var)(Cell_3D_IDs(1));
data(2:length(Cell_3D_IDs)+1,:) = mod.(var)(Cell_3D_IDs);
data(length(Cell_3D_IDs)+2,:) = mod.(var)(Cell_3D_IDs(length(Cell_3D_IDs)));
