...
def.cAxis(1).value = [0 700];
def.cAxis(2).value = [0 5]; 	%'WQ_DIAG_TOT_TN',...
def.cAxis(3).value = [0 .5]; 	%'WQ_DIAG_TOT_TP',...
def.cAxis(4).value = [0 25]; 	%'WQ_DIAG_TOT_TURBIDITY',...
def.cAxis(5).value = [0 1]; 	%'WQ_DIAG_TOT_TURBIDITY',...
def.cAxis(6).value = [0 1.5]; 	%'WQ_DIAG_TOT_TURBIDITY',...
...
outputdirectory = 'F:\Cloudstor\Shared\Aquatic Ecodynamics (AED)\AED_Lowerlakes\Coorong\Timeseries_Final\ORH_SLR\';
...
int = 1;
 ...
 ncfile(int).translate = 1;
  int = int + 1;
 ncfile(int).name = 'R:\Coorong-Output-16\ORH_SLR_02_20140101_20170101.nc';
 ncfile(int).symbol = {'-';'--'};
 ncfile(int).colour = {'b','b'}; % Surface and Bottom
 ncfile(int).legend = 'SLR 0.2m';
 ncfile(int).translate = 1;
  int = int + 1;
