...
if plotmodel
    for mod = 1:length(ncfile)
        ...
        tdata(mod).fields = tfv_readnetcdf(ncfile(mod).name,'timestep',1);
        all_cells(mod).X = double(tdata(mod).fields.cell_X);
        all_cells(mod).Y = double(tdata(mod).fields.cell_Y);
        ...
        d_data(mod).rawGeo = tdata(mod).fields;
...
for var = start_plot_ID:end_plot_ID
    ...
    for site = sites %1:length(shp)
        ...
        for mod = 1:length(ncfile)
            if isfield(tdata(mod).fields,loadname)
            if plotmodel
                tic
