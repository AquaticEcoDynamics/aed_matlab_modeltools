...
%     end
    ...
    for mod = 1:length(ncfile)
        raw(mod).data = load([ncfile(mod).name,varname{var},'.mat']);
        
        all_cells(mod).X = raw(mod).data.savedata.X;
        all_cells(mod).Y = raw(mod).data.savedata.Y;
        
        raw2(mod).data = load([ncfile(mod).name,'D','.mat']);
        d_data(mod).D = raw2(mod).data.savedata.D;
    ...
    for site = 1:length(shp)
        ...
        for mod = 1:length(ncfile)
            ...
            [data(mod),c_units,isConv] = tfv_getmodeldatapolygon_matfile(raw(mod).data,ncfile(mod).name,all_cells(mod).X,all_cells(mod).Y,shp(site).X,shp(site).Y,{loadname},d_data(mod).D,depth_range);
