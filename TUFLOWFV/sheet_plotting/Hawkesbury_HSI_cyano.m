...
sitelist = dir('/Volumes/AED/Hawkesbury/');

for kk = 18:18 %3:length(sitelist)
    
    main_dir = ['/Volumes/AED/Hawkesbury/',sitelist(kk).name,'/'];
    base_dir = ['/Volumes/AED/Hawkesbury/',sitelist(kk).name,'/Output/'];
...
%     end
    ...
    for i = 8:8 %3:length(dirlist)
        
        str = strsplit(dirlist(i).name,'_')
        
        if strcmpi(str{1},'Output') == 1
            if exist([main_dir,dirlist(i).name,'/HN_Cal_2013_2014_3D_wq_WQ.nc'],'file')
                if ~exist([main_dir,dirlist(i).name,'/proc_cyano_V.mat'],'file')
                disp([main_dir,dirlist(i).name,'/HN_Cal_2013_2014_3D_wq_WQ.nc']);
                
                
                %stop
                
                data = tfv_readnetcdf([main_dir,dirlist(i).name,'/HN_Cal_2013_2014_3D_wq_WQ.nc'],'names',{'V_x';'V_y';'D';'TEMP';'WQ_NIT_NIT';'WQ_NIT_AMM';'WQ_PHS_FRP';'cell_A';'cell_X';'cell_Y';'idx2'});
                ...
                dat = tfv_readnetcdf([main_dir,dirlist(i).name,'/HN_Cal_2013_2014_3D_wq_WQ.nc'],'time',1);
                data.tdate = dat.Time;
...
%                         end
                        ...
                        %------ TOTAL RISK                        
                        data.CYANO_fT(jj,ii) = fT;
                        data.CYANO_fP(jj,ii) = fP;
                        data.CYANO_fV(jj,ii) = mV;
                        data.CYANO_fS(jj,ii) = fS;
                        data.CYANO_fN(jj,ii) = fN;
