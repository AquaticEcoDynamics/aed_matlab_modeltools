            ...
            for jjj = 1:length(data)
                ...
                for jj = 1:length(u_years)
                    ...
                    for ii = 1:length(period)
                        ...
                        theyears = find(all_years == u_years(jj));
                        ...
                        if ~isempty(themonths)
                            ...
                            plotdate(inc,jjj) = datenum(u_years(jj),period(ii).m(1),01);
                            
                            if fluxplot == 1
                                % Include negitives in Calc
                                tt = find(isnan(ydata(theyears(themonths))) == 0);
                            else
                                % Exclude Negitives in Calc
                                tt = find(ydata(theyears(themonths)) > 0);
                            end
                            
                            
                            if inc == 1 % Cumulative Monthly Total
                                
                                plotdata(1,jjj) = sum(ydata(theyears(themonths(tt))) .* timeperiod);
                                
                                
                            else
                                
                                plotdata(inc,jjj) = plotdata(inc-1,jjj) + (sum(ydata(theyears(themonths(tt))) .* timeperiod));
...
%             end
            ...
            if strcmpi(sites{i},'Murray') == 1 & strcmpi(vars{j},'NIT_amm') == 1
