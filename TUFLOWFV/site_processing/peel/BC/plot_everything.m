...
% 
%     
% dirlist = dir(['BC_Files/','*.csv']);
% 
% for i = 1:length(dirlist)
%     disp(dirlist(i).name);
%     data.(['bc_',regexprep(dirlist(i).name,'.csv','')]) = tfv_readBCfile(['BC_Files/',dirlist(i).name]);
%     
% end
% 
...
datearray = datenum(1970:10:2020,01,01);
...
for i = 1:length(vars)
    ...
    if strcmpi(vars{i},'Date') == 0
        ...
        print(gcf,['Images_All/BC/',vars{i},'.png'],'-dpng','-zbuffer');
