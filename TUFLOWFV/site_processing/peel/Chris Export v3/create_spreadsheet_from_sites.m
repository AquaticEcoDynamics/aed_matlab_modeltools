...
for ccc = 3:length(modlist)
    ...
    for i = 1:length(shp)
        ...
        load([maindir,shp(i).Name,'/cell_A.mat']);
        ...
        load([maindir,shp(i).Name,'/WQ_TRC_AGE.mat']);
        ...
        load([maindir,shp(i).Name,'/WQ_NIT_NIT.mat']);
        
        nit = savedata;
        
        load([maindir,shp(i).Name,'/WQ_DIAG_MAG_HSI.mat']);
        
        maghsi = savedata;
        
        
        load([maindir,shp(i).Name,'/WQ_DIAG_PHY_TCHLA.mat']);
        
        tchla = savedata;
        
        load([maindir,shp(i).Name,'/WQ_DIAG_TOT_TN.mat']);
        
        tn = savedata;
        
        load([maindir,shp(i).Name,'/WQ_DIAG_TOT_TP.mat']);
        
        tp = savedata;
        
        load([maindir,shp(i).Name,'/WQ_PHS_FRP.mat']);
        
        frp = savedata;
        
        load([maindir,shp(i).Name,'/V_x.mat']);
        
        Vx = savedata;
        
        load([maindir,shp(i).Name,'/V_y.mat']);
...
%             end
            ...
            if ~isempty(depth.D)
                ...
                if ~isempty(hour_array)
                    ...
                    max_tchla = max(tchla.WQ_DIAG_PHY_TCHLA.Top(d_chx,hour_array_diag),tchla.WQ_DIAG_PHY_TCHLA.Bot(d_chx,hour_array_diag));
                    ...
                    for l = 1:length(day_array)
                        ...
                        mag_bot_days(l) = mean(maghsi.WQ_DIAG_MAG_HSI.Bot(d_chx,diag_ind));
                        ...
                        %%% WQI Catherines
                        wqi(l) = calc_hsi(oxy.WQ_OXY_OXY.Bot(d_chx,oxy_ind),tchla.WQ_DIAG_PHY_TCHLA.Top(d_chx,diag_ind),...
