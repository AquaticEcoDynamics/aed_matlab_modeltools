...
outdir = 'Matfiles/Peel_WQ_Model_v5_2016_2017_3D_Murray/';

% Shape file already imported as a mat file - In a shape file, each polygon can only have one value per attribute (Date), my polygons have dozens of dates, so there is some pre processing.
...
for i = 1:length(vars)
    ...
    for j = 1:length(shp)
        ...
        savedata.A = data.cell_A(fsite(j).theID);
        
        if strcmpi(vars{i},'H') == 1 | strcmpi(vars{i},'D') == 1
