...
for (k in 1:length(listcsv)){
 ndat[[k]] <- read.csv(listcsv[k])

filename <- paste0(listcsv[k])

grid.cell <- ndat[[k]]$Cell
age_bottom <- ndat[[k]]$Age
oxygen_bottom <- ndat[[k]]$Oxy
salinity_.bottom <- ndat[[k]]$Sal
Period <- ndat[[k]]$Period
a[[k]] <- data.frame(grid.cell,age_bottom,oxygen_bottom,salinity_.bottom,Period,stringsAsFactors=FALSE)

#predict scores (and SE) for dataframe using gam m1
a[[k]]$pred_FCI <- predict(m1, newdata=a[[k]], se.fit = TRUE)$fit
a[[k]]$FCI.se <- predict(m1, newdata=a[[k]], se.fit = TRUE)$se.fit  

#optional - add FCI grades to estimates
a[[k]] <- a[[k]]%>% mutate(grade = ifelse(pred_FCI < 10.4, "E", "D"),
                 grade = ifelse(pred_FCI >= 41, "C", grade),
                 grade = ifelse(pred_FCI >=54, "B", grade),
                 grade = ifelse(pred_FCI >= 71, "A", grade))

write.csv(file=paste0("E:\Github 2018\aed_matlab_modeltools\TUFLOWFV\site_processing\peel\FCI\output/",filename),a[[k]][,c("grid.cell","pred_FCI","FCI.se","grade")])
}
