...
ncfile = 'T:\PEEL\NEWER\run_2009_2010_rerun_Mickey.nc';


fid = fopen('run_2009_2010_rerun.csv','wt');
...
[~,t_ind] = min(abs(timesteps - datenum(2009,11,10,00,00,00)));
...
for i = 1:length(vars)
    switch vars{i}
        ...
        case 'U'
            cdata.(vars{i})(1:length(ID),1) = 0;
        case 'V'
            cdata.(vars{i})(1:length(ID),1) = 0;
        case 'SAL'
            cdata.(vars{i}) = rawGeo.(vars{i})(bottom_cells);
        case 'TRACE_1'
            cdata.(vars{i})(1:length(ID),1) = 0;
        otherwise
