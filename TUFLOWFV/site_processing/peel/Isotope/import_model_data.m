function [Adata,mTime,tdat] = import_model_data(ncfile,varname,sdate,edate)

dat = tfv_readnetcdf(ncfile,'time',1);

mTime = dat.Time;

sss = find(mTime >= sdate & mTime <= edate);

inc = 1;

for i = 1:length(sss)
    
    disp([num2str((i/length(sss))*100),'%']);
    
    tdat = tfv_readnetcdf(ncfile,'timestep',sss(i));clear functions

    temp= tdat.(varname)(tdat.idx3(tdat.idx3 > 0));
    Adata_A(:,inc) = temp;
    
    inc = inc + 1;
end

Adata = mean(Adata_A,2);
