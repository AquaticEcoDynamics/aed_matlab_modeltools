...
for i = 1:length(theorder)
    ...
    sss = find(phyHab.(theorder{i}).mdates >= datenum(themonths(1).year(1),themonths(1).val(1),01,00,00,00) & ...
        phyHab.(theorder{i}).mdates <= datenum(themonths(1).year(2),themonths(1).val(2),theday,23,59,59));
    
    if strcmpi(var,'WQ_OXY_OXY') == 1 & ...
            do_crit == 1
        delmap.raw.(theorder{i}) = calc_crit(phyHab.(theorder{i}).(var)(:,sss),conv);
    else
        
        delmap.raw.(theorder{i}) = mean(phyHab.(theorder{i}).(var)(:,sss),2) * conv;
