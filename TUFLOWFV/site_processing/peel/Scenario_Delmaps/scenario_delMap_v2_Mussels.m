...
load('Y:\Peel Final Report\HSI\musselHSI\scendata_musselHSI.mat');
...
for i = 1:length(theorder)
    ...
    sss = find(musselHSI.(theorder{i}).mdates >= datenum(themonths(1).year(1),themonths(1).val(1),01,00,00,00) & ...
        musselHSI.(theorder{i}).mdates <= datenum(themonths(1).year(2),themonths(1).val(2),theday,23,59,59));
    
    if strcmpi(var,'WQ_OXY_OXY') == 1 & ...
            do_crit == 1
        delmap.raw.(theorder{i}) = calc_crit(musselHSI.(theorder{i}).(var)(:,sss),conv);
    else
        
        delmap.raw.(theorder{i}) = mean(musselHSI.(theorder{i}).(var)(:,sss),2) * conv;
...
if ~do_crit
    saveas(gcf,[outdir,var,'_MUSSEL_',depth,'_',datetext,'_raw.png']);close;
else
...
if ~do_crit
    saveas(gcf,[outdir,var,'_MUSSEL_',depth,'_',datetext,'_delMap_minus_',base1,'.png']);close;
...
if ~do_crit
    saveas(gcf,[outdir,var,'_MUSSEL_',depth,'_',datetext,'_delMap_minus_',base2,'.png']);close;
