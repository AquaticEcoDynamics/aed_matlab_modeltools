...
Flow = interp1(canning.mdate,canning.flow,datearray,'linear',mean(canning.flow));
...
t_depth = swan.s616094.(varname).Depth;
tt = find(t_depth > -0.3);

[t_date,ind] = unique(swan.s616094.(varname).Date(tt));
t_data = swan.s616094.(varname).Data(tt(ind));
...
t_depth = swan.s6160262.(varname).Depth;
tt = find(t_depth > -0.3);

[t_date,ind] = unique(swan.s6160262.(varname).Date(tt));
t_data = swan.s6160262.(varname).Data(tt(ind));
