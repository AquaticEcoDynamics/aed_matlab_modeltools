...
datearray(:,1) = (sdate-1):15/(60*24):datenum(2020,12,31,11,59,00);
...
for i = 1:length(sites)
    ...
    if ~isempty(CHLA)
        GRN_IN = CHLA .* 0.04 * 16/106;
        IC.(sites{i}).GRN_IN.Data = GRN_IN;
        IC.(sites{i}).GRN_IN.Date = datearray;
        IC.(sites{i}).GRN_IN.X = X;
        IC.(sites{i}).GRN_IN.Y = Y;
    else
        GRN_IN = [];
    end
    if ~isempty(CHLA)
        GRN_IP = CHLA .* 0.04 * 1/106;
        IC.(sites{i}).GRN_IP.Data = GRN_IP;
        IC.(sites{i}).GRN_IP.Date = datearray;
        IC.(sites{i}).GRN_IP.X = X;
        IC.(sites{i}).GRN_IP.Y = Y;
    else
        GRN_IP = [];
    end 
    ...
    if ~isempty(CHLA)
        GRN_RHO(1:length(CHLA)) = 1000;
        IC.(sites{i}).GRN_RHO.Data = GRN_RHO;
        IC.(sites{i}).GRN_RHO.Date = datearray;
        IC.(sites{i}).GRN_RHO.X = X;
        IC.(sites{i}).GRN_RHO.Y = Y;
    else
        GRN_RHO = [];
    end 
    ...
    if ~isempty(CHLA)
        BGA_IN = CHLA .* 0.04 * 16/106;
        IC.(sites{i}).BGA_IN.Data = BGA_IN;
        IC.(sites{i}).BGA_IN.Date = datearray;
        IC.(sites{i}).BGA_IN.X = X;
        IC.(sites{i}).BGA_IN.Y = Y;
    else
        BGA_IN = [];
    end
    if ~isempty(CHLA)
        BGA_IP = CHLA .* 0.04 * 1/106;
        IC.(sites{i}).BGA_IP.Data = BGA_IP;
        IC.(sites{i}).BGA_IP.Date = datearray;
        IC.(sites{i}).BGA_IP.X = X;
        IC.(sites{i}).BGA_IP.Y = Y;
    else
        BGA_IP = [];
    end 
    ...
    if ~isempty(CHLA)
        BGA_RHO(1:length(CHLA)) = 1000;
        IC.(sites{i}).BGA_RHO.Data = GRN_RHO;
        IC.(sites{i}).BGA_RHO.Date = datearray;
        IC.(sites{i}).BGA_RHO.X = X;
        IC.(sites{i}).BGA_RHO.Y = Y;
    else
        BGA_RHO = [];
    end 
    ...
    if ~isempty(CHLA)
        FDIAT_IN = CHLA .* 0.04 * 16/106;
        IC.(sites{i}).FDIAT_IN.Data = FDIAT_IN;
        IC.(sites{i}).FDIAT_IN.Date = datearray;
        IC.(sites{i}).FDIAT_IN.X = X;
        IC.(sites{i}).FDIAT_IN.Y = Y;
    else
        FDIAT_IN = [];
    end
    if ~isempty(CHLA)
        FDIAT_IP = CHLA .* 0.04 * 1/106;
        IC.(sites{i}).FDIAT_IP.Data = FDIAT_IP;
        IC.(sites{i}).FDIAT_IP.Date = datearray;
        IC.(sites{i}).FDIAT_IP.X = X;
        IC.(sites{i}).FDIAT_IP.Y = Y;
    else
        FDIAT_IP = [];
    end 
    ...
    if ~isempty(CHLA)
        FDIAT_RHO(1:length(CHLA)) = 1000;
        IC.(sites{i}).FDIAT_RHO.Data = FDIAT_RHO;
        IC.(sites{i}).FDIAT_RHO.Date = datearray;
        IC.(sites{i}).FDIAT_RHO.X = X;
        IC.(sites{i}).FDIAT_RHO.Y = Y;
    else
        FDIAT_RHO = [];
    end 
    ...
    if ~isempty(CHLA)
        MDIAT_IN = CHLA .* 0.04 * 16/106;
        IC.(sites{i}).MDIAT_IN.Data = MDIAT_IN;
        IC.(sites{i}).MDIAT_IN.Date = datearray;
        IC.(sites{i}).MDIAT_IN.X = X;
        IC.(sites{i}).MDIAT_IN.Y = Y;
    else
        MDIAT_IN = [];
    end
    if ~isempty(CHLA)
        MDIAT_IP = CHLA .* 0.04 * 1/106;
        IC.(sites{i}).MDIAT_IP.Data = MDIAT_IP;
        IC.(sites{i}).MDIAT_IP.Date = datearray;
        IC.(sites{i}).MDIAT_IP.X = X;
        IC.(sites{i}).MDIAT_IP.Y = Y;
    else
        MDIAT_IP = [];
    end 
    ...
    if ~isempty(CHLA)
        MDIAT_RHO(1:length(CHLA)) = 1000;
        IC.(sites{i}).MDIAT_RHO.Data = MDIAT_RHO;
        IC.(sites{i}).MDIAT_RHO.Date = datearray;
        IC.(sites{i}).MDIAT_RHO.X = X;
        IC.(sites{i}).MDIAT_RHO.Y = Y;
    else
        MDIAT_RHO = [];
    end 
    ...
    if ~isempty(CHLA)
        KARLO_IN = CHLA .* 0.04 * 16/106;
        IC.(sites{i}).KARLO_IN.Data = KARLO_IN;
        IC.(sites{i}).KARLO_IN.Date = datearray;
        IC.(sites{i}).KARLO_IN.X = X;
        IC.(sites{i}).KARLO_IN.Y = Y;
    else
        KARLO_IN = [];
    end
    if ~isempty(CHLA)
        KARLO_IP = CHLA .* 0.04 * 1/106;
        IC.(sites{i}).KARLO_IP.Data = KARLO_IP;
        IC.(sites{i}).KARLO_IP.Date = datearray;
        IC.(sites{i}).KARLO_IP.X = X;
        IC.(sites{i}).KARLO_IP.Y = Y;
    else
        KARLO_IP = [];
    end 
    ...
    if ~isempty(CHLA)
        KARLO_RHO(1:length(CHLA)) = 1000;
        IC.(sites{i}).KARLO_RHO.Data = KARLO_RHO;
        IC.(sites{i}).KARLO_RHO.Date = datearray;
        IC.(sites{i}).KARLO_RHO.X = X;
        IC.(sites{i}).KARLO_RHO.Y = Y;
    else
        KARLO_RHO = [];
    end 
    
    
if isfield(IC.(sites{i}),'FDIAT')
    IC.(sites{i}).CRYPT = IC.(sites{i}).FDIAT; 
    IC.(sites{i}).CRYPT_IN = IC.(sites{i}).KARLO_IN;
IC.(sites{i}).CRYPT_IP = IC.(sites{i}).KARLO_IP;
IC.(sites{i}).CRYPT_RHO = IC.(sites{i}).KARLO_RHO;
end
if isfield(IC.(sites{i}),'MDIAT')
IC.(sites{i}).DIATOM = IC.(sites{i}).MDIAT;
IC.(sites{i}).DIATOM_IN = IC.(sites{i}).MDIAT_IN;
IC.(sites{i}).DIATOM_IP = IC.(sites{i}).MDIAT_IP;
IC.(sites{i}).DIATOM_RHO = IC.(sites{i}).MDIAT_RHO;
end

if isfield(IC.(sites{i}),'KARLO')
IC.(sites{i}).DINO = IC.(sites{i}).KARLO; 
IC.(sites{i}).DINO_IN = IC.(sites{i}).KARLO_IN;
IC.(sites{i}).DINO_IP = IC.(sites{i}).KARLO_IP;
IC.(sites{i}).DINO_RHO = IC.(sites{i}).KARLO_RHO;
end
% ________________________________
    ...
    IC.(sites{i}).AGE.Data(1:length(datearray)) = 0;
    IC.(sites{i}).AGE.Date = datearray;
    IC.(sites{i}).AGE.X = X;
    IC.(sites{i}).AGE.Y = Y;
